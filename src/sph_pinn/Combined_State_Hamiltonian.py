...
1.  State Prediction Networks: Two MLPs with Fourier features (HR_Net and Error_Net)
    learn the state trajectories q(t) and s(t) respectively.
...
# ==============================================================================
# 1. NEURAL NETWORK DEFINITIONS (UNCHANGED)
# ==============================================================================

# --- State Prediction Networks (from PINN implementation) ---
...
class HRStateNN(eqx.Module):
    """An MLP with Fourier Features to approximate the HR state q(t)."""
    layers: list

    def __init__(self, key, out_size=10, width=128, depth=6, mapping_size=128, scale=500):
        ...
        self.layers = [
            ...
            *[eqx.nn.Linear(width, width, key=layer_keys[i]) for i in range(1, depth - 1)],
...
class ErrorStateNN(eqx.Module):
    """An MLP with Fourier Features to approximate the ERROR state s(t)."""
    layers: list

    def __init__(self, key, out_size=5, width=128, depth=6, mapping_size=128, scale=500):
        fourier_key, *layer_keys = jax.random.split(key, depth + 1)
        self.layers = [
            FourierFeatures(fourier_key, in_size=1, mapping_size=mapping_size, scale=scale),
            eqx.nn.Linear(mapping_size, width, key=layer_keys[0]),
...
class Combined_sPHNN_PINN(eqx.Module):
    """Main model combining state predictors and sPHNN structure."""
    hr_net: HRStateNN
    error_net: ErrorStateNN
    hamiltonian_net: HamiltonianNN
    dissipation_net: DissipationNN
    j_net: DynamicJ_NN

    def __init__(self, key, config):
        hr_key, error_key, h_key, d_key, j_key = jax.random.split(key, 5)
        state_dim = config['state_dim']
        # The equilibrium point for the normalized error system is the origin.
        x0_norm = jnp.zeros(state_dim)

        self.hr_net = HRStateNN(
            key=hr_key, out_size=config['q_dim'], width=128, depth=6, mapping_size=128, scale=500
        )
        self.error_net = ErrorStateNN(
            key=error_key, out_size=state_dim, width=128, depth=4, mapping_size=128, scale=500
        )
...
@eqx.filter_jit
def loss_fn(model: Combined_sPHNN_PINN, t_batch_norm, s_true_batch_norm, q_true_batch_norm,
    ...
    # --- Part 1: State Prediction and Data Fidelity Loss ---
    q_pred_batch_norm = jax.vmap(model.hr_net)(t_batch_norm)
    s_pred_batch_norm = jax.vmap(model.error_net)(t_batch_norm)

    loss_q_data = jnp.mean((q_pred_batch_norm - q_true_batch_norm) ** 2)
    loss_s_data = jnp.mean((s_pred_batch_norm - s_true_batch_norm) ** 2)
    data_loss = loss_q_data + loss_s_data

    # Denormalize states for physics calculations
    ...
    # Conservative Loss: L_fc(H) = f_c . grad(H) = 0
    lie_derivative = jax.vmap(jnp.dot)(f_c_batch, grad_H)
    loss_conservative = jnp.mean(lie_derivative ** 2)

    # Dissipative Loss: dH/dt_structure = dH/dt_equations
    dHdt_from_structure = jax.vmap(jnp.dot)(grad_H, s_dot_from_structure)
    dHdt_from_equations = jax.vmap(jnp.dot)(grad_H, f_d_batch)
    loss_dissipative = jnp.mean((dHdt_from_structure - dHdt_from_equations) ** 2)

    # --- MODIFICATION STARTS HERE ---

    # Get the analytical vector field using the specified complete function
    s_dot_analytical = jax.vmap(
        HRNetworkErrorSystem.complete_vector_field, in_axes=(0, 0, None)
    )(s_pred, q_pred, hr_params)

    # Re-introduce s_dot_diss_cons as the sum of the helper functions
    s_dot_diss_cons = f_c_batch + f_d_batch

    # Calculate physics structure loss using the user-specified formula
    loss_physics_structure = jnp.mean((s_dot_from_structure - s_dot_analytical - s_dot_diss_cons) ** 2)
    ...
    total_loss = (data_loss
                  ...
                  + lambda_physics * loss_physics_structure)
    ...
    loss_components = {
        ...
        "physics_structure": loss_physics_structure,
    }
    return total_loss, loss_components


@eqx.filter_jit
def train_step(model, opt_state, optimizer, t_batch_norm, s_batch_norm, q_batch_norm,
    ...
    (loss_val, loss_components), grads = eqx.filter_value_and_grad(loss_fn, has_aux=True)(
        model, t_batch_norm, s_batch_norm, q_batch_norm,
        lambda_conservative, lambda_dissipative, lambda_physics, hr_params, 
        t_mean, t_std, s_mean, s_std, q_mean, q_std
    )
...
@eqx.filter_jit
def evaluate_model(model, t_batch_norm, s_batch_norm, q_batch_norm,
                   lambda_conservative, lambda_dissipative, lambda_physics, hr_params, 
                   t_mean, t_std, s_mean, s_std, q_mean, q_std):
...
# ==============================================================================
# 4. MAIN EXECUTION LOGIC (MODIFIED)
# ==============================================================================
...
t_shuffled, s_shuffled, q_shuffled, H_shuffled = t[perm], s[perm], q[perm], H_analytical[perm]
...
nn_config = {
    "state_dim": s_dim, "q_dim": q_dim,
    "h_width": 128, "h_depth": 3, "h_epsilon": 1e-6,
    "d_width": 128, "d_depth": 3, "j_width": 128, "j_depth": 3,
...
conservative_losses, dissipative_losses, physics_structure_losses = [], [], []
...
for epoch in range(epochs):
    ...
    t_shuffled, s_shuffled, q_shuffled = t_train_norm[perm], s_train_norm[perm], q_train_norm[perm]

    # Initialize epoch loss accumulators
    epoch_losses = {k: 0.0 for k in ["total", "data_s", "data_q", "conservative", "dissipative", "physics_structure"]}
    
    for i in range(num_batches):
        start, end = i * batch_size, (i + 1) * batch_size
        t_b, s_b, q_b = t_shuffled[start:end], s_shuffled[start:end], q_shuffled[start:end]

        model, opt_state, train_loss_val, loss_comps = train_step(
            model, opt_state, optimizer, t_b, s_b, q_b,
    ...
    val_loss = evaluate_model(
        model, t_val_norm, s_val_norm, q_val_norm,
        current_lambda_conservative, current_lambda_dissipative, current_lambda_physics, hr_params,
        t_mean, t_std, s_mean, s_std, q_mean, q_std
    )
    ...
    physics_structure_losses.append(avg_losses["physics_structure"])
    ...
    if (epoch + 1) % 100 == 0 or epoch == 0:
        log_str = (
            ...
            f"Data(s): {avg_losses['data_s']:.4f} | Data(q): {avg_losses['data_q']:.4f} | "
            f"Cons: {avg_losses['conservative']:.4f} | Diss: {avg_losses['dissipative']:.4f} | "
            f"Struct: {avg_losses['physics_structure']:.4f}"
        )
        print(log_str)
...
# ==============================================================================
# 5. VISUALIZATION AND ANALYSIS (MODIFIED FOR NEW LOSSES)
# ==============================================================================
...
s_pred_norm = jax.vmap(best_model.error_net)(t_test_norm)
q_pred_norm = jax.vmap(best_model.hr_net)(t_test_norm)
...
get_autodiff_grad = lambda net, t: jax.jvp(net, (t,), (jnp.ones_like(t),))[1]
s_dot_autodiff_norm = jax.vmap(get_autodiff_grad, in_axes=(None, 0))(best_model.error_net, t_test_norm)
...
correlation = jnp.corrcoef(H_analytical_vis, H_learned_norm.flatten())[0, 1]
sign = jnp.sign(correlation)
H_learned_flipped = sign * H_learned_norm
...
plt.plot(t_test[:2000], H_learned_aligned[:2000], label='Learned Hamiltonian (Aligned)', color='red', linestyle='--')
...
plt.plot(physics_structure_losses, label='Physics Structure Loss', alpha=0.7, linestyle='-')
plt.plot(conservative_losses, label='Conservative Loss', alpha=0.7, linestyle='--')
plt.plot(dissipative_losses, label='Dissipative Loss', alpha=0.7, linestyle=':')
plt.yscale('log')
...
fig.suptitle("Derivative Fidelity: True vs. sPHNN vs. Analytical Physics vs. Autodiff", fontsize=18, y=0.99)
for i in range(s_test.shape[1]):
    ...
    axes[i].plot(t_test[:2000], s_dot_from_structure[:2000, i], label=r'$\dot{s}_{structure}$', color='red', linestyle='--')
    axes[i].plot(t_test[:2000], s_dot_from_equations[:2000, i], label=r'$\dot{s}_{equations}$', color='purple', linestyle=':')
    axes[i].plot(t_test[:2000], s_dot_autodiff[:2000, i], label=r'$\dot{s}_{autodiff}$', color='orange', linestyle='-.')
